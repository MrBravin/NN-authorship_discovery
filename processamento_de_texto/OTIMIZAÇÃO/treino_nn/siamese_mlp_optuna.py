...
class SiameseMLP(nn.Module):
    def __init__(self, in_dim: int, width: int, depth: int, proj_dim: int, activation: str = "relu",
        ...
        self.temperature = nn.Parameter(torch.tensor(float(temperature)), requires_grad=False)  # fixo no treino
...
def objective_factory(v1_tr_full, v2_tr_full, y_tr_full, in_dim: int, seed: int = SEED):
    def objective(trial: optuna.Trial) -> float:
        ...
        use_layernorm = trial.suggest_categorical("use_layernorm", [True])  # fixado True (estável); op: [True, False]
